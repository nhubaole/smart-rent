// ignore_for_file: constant_identifier_names

class KeyValue {
  // STATUS AUTHENTICATION
  static const KEY_STATUS_LOGIN = 'isLogin';

  // ACCOUNT
  static const KEY_COLLECTION_ACCOUNT = 'ACCOUNT_COLLECTION';
  static const KEY_COLLECTION_ROOM = 'ROOM_COLLECTION';
  static const KEY_STORAGE_ACCOUNT_IMG = 'ACCOUNT_IMG';
  static const API_GOOGLE_MAP = 'AIzaSyBC5XsEcx90TU2pWq2iTPCBN58dTKUmKSM';
  static const KEY_COLLECTION_MESSAGE = 'MESSAGE_COLLECTION';
  static const KEY_COLLECTION_ROOM_MESSAGE = 'ROOM_MESSAGE_COLLECTION';
  static const KEY_COLLECTION_ROOM_USER = 'ROOM_USER_COLLECTION';
  static const KEY_ACCOUNT_USERNAME = 'username';
  static const KEY_ACCOUNT_UID = 'uid';
  static const KEY_ACCOUNT_PHONENUMBER = 'phoneNumber';
  static const KEY_ACCOUNT_EMAIL = 'email';
  static const KEY_ACCOUNT_PHOTOURL = 'photoUrl';
  static const KEY_ACCOUNT_SEX = 'sex';
  static const KEY_ACCOUNT_DATEOFBIRTH = 'dateOfBirth';
  static const KEY_ACCOUNT_ADDRESS = 'address';
  static const KEY_ACCOUNT_AGE = 'age';
  static const KEY_ACCOUNT_DATEOFCREATE = 'dateOfCreate';
  static const KEY_ACCOUNT_ISONLINE = 'isOnline';

  // notify setting
  static const KEY_NOTIFY_NEWROOM = 'nofityNewRoom';
  static const KEY_NOTIFY_NEWMESSAGE = 'nofityNewMessage';
  static const KEY_NOTIFY_NEWLIKE = 'nofityNewLike';
  static const KEY_NOTIFY_NEWSCHEDULE = 'nofityNewSchedule';

  static const KEY_CURRENT_LOCATION = 'currentLocation';

  // status room
  static const KEY_ROOM_REQUEST_RENT = 'request_rent';
  static const KEY_ROOM_REQUEST_RETURN = 'request_return';
  static const KEY_ROOM_RENTED = 'rented';
  static const KEY_ROOM_POSTED = 'posted';
  static const KEY_ROOM_PENDING_APPROVED = 'pending';

  // payments
  static const KEY_ROOM_LIST_RECENTLY = 'room_list_recently';
  static const KEY_INVOICE_COLLECTION = 'INVOICE_COLLECTION';
  static const KEY_TICKET_REVIEW_COLLECTION = 'REVIEW_TICKET_COLLECTION';
  static const KEY_PAYOS_REQUEST_COLLECTION = 'PAYOS_REQUEST_COLLECTION';

  //
  static const KEY_DEVICE_COLLECTION = 'DEVICE_COLLECTION';
  static const KEY_NOTIFICATION_COLLECTION = 'NOTIFICATION_COLLECTION';

  // RETURN RENT
  static const KEY_TICKET_REQUEST_RETURN_RENT_ROOM_COLLECTION =
      'TICKET_REQUEST_RETURN_RENT_ROOM_COLLECTION';

  // REQUEST RENT
  static const KEY_TICKET_REQUEST_REQUEST_RENT_ROOM_COLLECTION =
      'TICKET_REQUEST_REQUEST_RENT_ROOM_COLLECTION';

  // HISTORY ROOM
  static const KEY_HISTORY_RENT_ROOM_COLLECTION =
      'HISTORY_RENT_ROOM_COLLECTION';
}
